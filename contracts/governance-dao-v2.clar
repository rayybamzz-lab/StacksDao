...
;; @desc get-proposal-votes-for
;; @param proposal-id uint
;; @returns (response uint uint)
;; Read-only context viewer
(define-read-only (get-proposal-votes-for (proposal-id uint))
  (match (map-get? proposals proposal-id)
    proposal (ok (get votes-for proposal))
    ERR-PROPOSAL-NOT-FOUND
  )
)

;; @desc get-proposal-votes-against
;; @param proposal-id uint
;; @returns (response uint uint)
;; Read-only context viewer
(define-read-only (get-proposal-votes-against (proposal-id uint))
  (match (map-get? proposals proposal-id)
    proposal (ok (get votes-against proposal))
    ERR-PROPOSAL-NOT-FOUND
  )
)
...
;; @desc get-winning-votes
;; @param proposal-id uint
;; @returns (response uint uint)
;; Read-only context viewer
(define-read-only (get-winning-votes (proposal-id uint))
  (match (map-get? proposals proposal-id)
    proposal (ok (if (> (get votes-for proposal) (get votes-against proposal)) (get votes-for proposal) (get votes-against proposal)))
    ERR-PROPOSAL-NOT-FOUND
  )
)
